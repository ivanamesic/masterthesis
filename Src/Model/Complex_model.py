...
results_dir = const.tezos_results_dir + "Predictions/Complex/"
...
dest_file_predictions = results_dir + "Predictions_complex_no_segmentation.npy"

print("Destination file " + dest_file_predictions)
...
files_exist = False
done_indexes = []
# if os.path.exists(dest_file_results) and os.path.exists(dest_file_predictions):
#     files_exist = True
#     prev_metrics = pd.read_csv(dest_file_results)
#     prev_predictions = pd.DataFrame(np.load(dest_file_predictions, allow_pickle=True))

#     done_indexes = prev_metrics.number.tolist() if len(prev_metrics.number.tolist()) <= len(prev_predictions.number.tolist()) else prev_predictions.number.tolist()
